<script lang="ts">
  ...
  $: currentSite = $team.sites.find((site) => site.id === currentSiteId);
...
{#if currentSite}
  ...
  <a
    ...
    aria-current="page"
    href={`/${$team.id}/${currentSite.id}`}
  >
    {#if currentSite.logo}
      <Img style={css.raw({ size: '20px' })} $image={currentSite.logo} alt={`${currentSite.name}의 로고`} size={24} />
    {/if}
