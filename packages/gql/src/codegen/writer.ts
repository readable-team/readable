...
export const writeArtifactAssets = async (
  ...
  for (const [name, operation] of operationMap) {
    ...
    await writeFile(path.join(gqlDir, `artifacts/operations/${name}.ts`), content);
  }
  ...
  for (const [name, fragment] of fragmentMap) {
    ...
    const content = AST.print(program);
    await writeFile(path.join(gqlDir, `artifacts/fragments/${name}.ts`), content);
  }
};

export const writePublicAssets = async (gqlDir: string, artifacts: Artifact[]) => {
...
export const writeMiscAssets = async (gqlDir: string) => {
